...
\addcontentsline{toc}{chapter}{Lessons}
	...
	\begin{parts}
		...
		\item Find a matrix $P_1$ whose $i,j$ entry represents the probability
			of transitioning from state $i$ to state $j$ in exactly one step.
		\item Find a matrix $P_2$ whose $i,j$ entry represents the probability
			of transitioning from state $i$ to state $j$ in exactly two steps.
	...
	\begin{definition}[Invariant Set]
		...
		$(f_1,\ldots,f_n)$ and non-zero probabilities $(p_1,\ldots,p_n)$.
	...
	\question
	Let $\mathcal I$ be the iterated function system with probabilities $(1/2,1/2)$
	...
	\begin{parts}
		...
		at unit speed counter-clockwise around your path.
		...
		\end{center}
			Let $S$ be the points on the surface of the lake and let $\Phi^t:F\to F$ be the result
			of ``flowing'' along the surface for $t$ seconds. Is $(\Phi^t, F)$ a continuous time dynamical system?
	\end{parts}

	Systems like $(\Phi^t,F)$ come up often because they are described by \emph{local} rules. That is, recorded in the
	...
	\question
		...
		the vector field with velocity at a given point equal to the magnitude of the vector at that point.
	\begin{parts}
	...
	\begin{parts}
		...
		\item If possible, find a $V$ so that the forward orbit of every point ends up at $\vec 0$ or $\vec e_1$.
	\end{parts}

	\begin{definition}[Stable \& Unstable Points]
		Let $(W^t,\R^n)$ be a continuous time dynamical system. We call the point $\vec x\in \R^n$ \emph{stable}
		if for all $\varepsilon>0$, there exists a $\delta>0$ so that $\Norm{\vec x-\vec y} < \delta$ implies
		\[
