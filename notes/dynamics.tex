...
\addcontentsline{toc}{chapter}{Lessons}
	...
	\begin{parts}
		...
		\item Use an online plotter (for example \url{https://www.desmos.com/calculator/eijhparfmd} ) to
			plot $V_X$ for $X\in\{I,A,B,C,D,E\}$.
		\item Compute the eigenvalues for each of the matrices. Can you relate the eigenvalues to 
			stable/unstable points?
	\end{parts}

	\subsection*{Differential Equations}
	To really get a handle on what's going on, let's thing about some differential equations!
	...
	\begin{parts}
		...
		\item Suppose $A$ is a diagonalizable matrix and $A=PDP^{-1}$ where $D$ is diagonal. Further,
			suppose $f$ is a solution to $f'(t)=Df(t)$. Find a solution to the differential equation
			$\Phi'(t)=A\Phi(t)$.
		\item Let $M$ be a matrix and consider the dynamical system coming from the vector field $\vec x\mapsto M\vec x$.
